...
export default function Home() {
  ...
  return (
    <main className="w-screen h-screen overflow-hidden bg-black" style={{ cursor: 'none' }}>
      ...
      {screen === 'landing' && (
        <Landing onEnter={handleLandingEnter} />
      )}
      {screen === 'ethical-gate' && (
        <EthicalGate onContinue={handleEthicalGateContinue} />
      )}
      {screen === 'birth-data' && (
        <BirthDataEntry onSubmit={handleBirthDataSubmit} />
      )}
      {screen === 'natal-seal' && birthData && (
        <NatalSeal birthData={birthData} onComplete={handleNatalSealComplete} />
      )}
      {screen === 'chart' && birthData && (
        <ChartEnvironment birthData={birthData} />
      )}
    </main>
  )
}
